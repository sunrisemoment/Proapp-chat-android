...
import sam.puma.proapp.utils.MimeUtils;
...
public class Message extends AbstractEntity {
	 ...
	 * anton inserted code
	 */
	public  boolean command = false; //message is command message or not ?
	...
	private Message(final String uuid, final String conversationUUid, final Jid counterpart,
					...
					final String edited, final boolean oob,final boolean command) {
		this.uuid = uuid;
	...
	public static Message fromCursor(Cursor cursor) {
		...
		return new Message(cursor.getString(cursor.getColumnIndex(UUID)),
				...
				cursor.getInt(cursor.getColumnIndex(IS_COMMAND)) > 0
		);
	}
	...
	public ContentValues getContentValues() {
		...
		values.put(IS_COMMAND,command ? 1 : 0);//if message was typed by command button, then this is true;
		return values;
	}
