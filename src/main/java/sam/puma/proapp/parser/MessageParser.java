...
public class MessageParser extends AbstractParser implements
	...
	public void onMessagePacketReceived(Account account, MessagePacket original) {
		...
		/**
		 * anton's code for message is command or not , then display star images
		 */
		boolean isCommand  = false;
		String receivedBody = "";
		if((packet != null)) {
			if (packet.getBody() != null) {
				if(packet.getBody().contains("::command")) {
					receivedBody = packet.getBody().split("::")[0];
					isCommand = true;
				}else{
					receivedBody = packet.getBody();
					isCommand = false;
				}
			} else {
				receivedBody = packet.getBody();
		...
		final String body = receivedBody;
		/*****************************************************************/
			...
			if (replacementId != null && mXmppConnectionService.allowMessageCorrection()) {
				...
				if (replacedMessage != null) {
					...
					if (fingerprintsMatch && (trueCountersMatch || conversation.getMode() == Conversation.MODE_SINGLE)) {
						...
						replacedMessage.command = isCommand;
