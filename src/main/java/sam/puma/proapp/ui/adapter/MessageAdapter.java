...
import sam.puma.proapp.entities.Message.FileParams;
import sam.puma.proapp.entities.Transferable;
import sam.puma.proapp.ui.ConversationActivity;
...
public class MessageAdapter extends ArrayAdapter<Message> {
	...
	private void displayStatus(ViewHolder viewHolder, Message message, int type, boolean darkBackground) {
		...
		switch (message.getMergedStatus()) {
			...
			default:
				if (multiReceived) {
					String contact = UIHelper.getMessageDisplayName(message);
					if(contact.contains("_")){
						info = contact.split("_")[0]+"@"+contact.split("_")[1];
					} else {
						info = contact;
					}
				}
		...
		if (view == null) {
			...
			switch (type) {
				case SENT:
					...
					/**
					 * if message was typed by command button then it must be star image in message view cell;
					 */
					if (message.command) {
						viewHolder.starImage.setVisibility(View.VISIBLE);
					}
					else {
						viewHolder.starImage.setVisibility(View.GONE);
					}
			...
			/**
			 * if message type is status then star image must be disapper
			 */
