...
public class ConversationAdapter extends ArrayAdapter<Conversation> {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		if (conversation.getMode() == Conversation.MODE_SINGLE || activity.useSubjectToIdentifyConference()) {
			/**
			 * anton's code for display conversation name on conversation overview
			 */
			if(conversation.getName().contains("_")){
				convName.setText(conversation.getName().split("_")[0]);
			} else {
				convName.setText(conversation.getName());
			}
		} else {
			if(conversation.getJid().toBareJid().toString().contains("_")){
				convName.setText(conversation.getJid().toBareJid().toString().split("_")[0]);
			} else {
