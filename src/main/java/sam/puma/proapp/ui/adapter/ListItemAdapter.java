...
public class ListItemAdapter extends ArrayAdapter<ListItem> {
	...
	public View getView(int position, View view, ViewGroup parent) {
		...
		/**
		 * for display jid is in list.
		 */
		final String jid;
		if(item.getDisplayJid().contains("_")){
			jid = item.getDisplayJid().split("@")[0].split("_")[0]+"@"+item.getDisplayJid().split("@")[0].split("_")[1];
		}else{
			jid = item.getDisplayJid().split("@")[0];
		}
		if(item.getDisplayName().contains("_")){
			tvName.setText(item.getDisplayName().split("@")[0].split("_")[0]);
		}else{
			tvName.setText(item.getDisplayName().split("@")[0]);
		}
		/************************************************************************************/
		if (jid != null) {
