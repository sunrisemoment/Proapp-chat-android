	...
	public EnterJidDialog(
		...
		this.dialogOnClick = new View.OnClickListener() {
			...
			public void onClick(final View v) {
				...
				try {
					/**
					 * anton's code for display contact create dialog text field.
					 */
				String accountJidString;
					...
					if (Config.DOMAIN_LOCK != null) {
						accountJid = Jid.fromParts(accountJidString,Config.DOMAIN_LOCK,null);
					} else {
						accountJid = Jid.fromString(accountJidString);
					}
					/*********************************************************************/
				...
				String jidString = null;
				try {
					if(jid.getText().toString().contains("@")){
						jidString = jid.getText().toString().split("@")[0]+"_"+jid.getText().toString().split("@")[1]+"@"+"54.169.8.76";
					} else {
						jidString = jid.getText().toString()+"@"+"54.169.8.76";
					}
					if (lock) {
						contactJid = Jid.fromParts(jidString, Config.DOMAIN_LOCK, null);
					} else {
						contactJid = Jid.fromString(jidString);
					}
